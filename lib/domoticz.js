...
Domoticz.settings = function(accessory, completion, error) {
  ...
  var url = accessory.platform.apiBaseURL + "type=command&param=getsettings";
  baseHttpRequest.get({
    url: url
...
Domoticz.devices = function(baseURL, roomID, completion, error) {
  ...
  var url = baseURL + "type=command&param=getdevices&used=true&order=Name";
  if (roomID) {
      url += "&plan=" + roomID;
  }
...
Domoticz.hardware = function(baseURL, completion, error) {
  ...
  var url = baseURL + "type=command&param=gethardware";
...
Domoticz.deviceStatus = function(accessory, completion, error) {
  ...
  var url = accessory.platform.apiBaseURL + "type=command&param=getdevices&rid=" + accessory.idx;
  baseHttpRequest.get({
    url: url
  }, function(err, response, json) {
    if (!err && response.statusCode == 200 && json !== undefined)
